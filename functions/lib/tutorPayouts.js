...
exports.markPayoutPaid = exports.reviewPayoutRequest = exports.cancelPayoutRequest = exports.requestPayout = exports.saveTutorPayoutDetails = void 0;
...
exports.markPayoutPaid = functionsV1
    ...
    const result = await db.runTransaction(async (tx) => {
        ...
        const balance = balanceSnap.exists ? Number(balanceSnap.data()?.balance ?? 0) : 0;
        if (requestedAmount > balance) {
            throw new functionsV1.https.HttpsError("failed-precondition", `Tutor's balance (₹${balance}) is now less than the requested amount (₹${requestedAmount}) — cannot mark paid`);
        }
        const now = admin.firestore.FieldValue.serverTimestamp();
