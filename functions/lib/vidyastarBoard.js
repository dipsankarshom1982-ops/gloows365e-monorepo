...
exports.onContestParticipantWrite = void 0;
...
exports.onContestParticipantWrite = (0, firestore_1.onDocumentWritten)({ document: "contests/{contestId}/participant/{userId}" }, async (event) => {
    ...
    if (!after?.completed)
        return null;
    const { contestId, userId } = event.params;
    ...
    try {
        ...
        if (!contestSnap.exists)
            return null;
        const periodKey = contestSnap.data()?.periodKey;
        if (!periodKey)
            return null;
        ...
        let totalScore = 0;
        let contestCount = 0;
        await Promise.all(contestsSnap.docs.map(async (cDoc) => {
            try {
...
async function recalculateVidyastarRanks(periodKey, db) {
    try {
...
//# sourceMappingURL=vidyastarBoard.js.map