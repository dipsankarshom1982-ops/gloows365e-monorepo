...
exports.submitVidyastarContestQuiz = void 0;
const admin = require("firebase-admin");
const functionsV1 = require("firebase-functions/v1");
const db = admin.firestore();
const LEADERBOARD_TABS = ["daily", "weekly", "monthly", "yearly"];
...
async function getContestRewardAmount() {
    try {
        const snap = await db.doc(`vCoinRules/${VIDYASTAR_CONTEST_ENTRY}`).get();
        if (snap.exists) {
            const rule = snap.data();
            if (rule.isActive && typeof rule.rewardAmount === "number")
                return rule.rewardAmount;
        }
    }
    catch { /* fall through to default */ }
    return DEFAULT_CONTEST_REWARD;
}
async function syncToStarboard(uid, score) {
    if (score <= 0)
        return;
    let name = "Student";
    let state;
...
async function awardContestVCoins(uid, contestId) {
...
exports.submitVidyastarContestQuiz = functionsV1
    ...
    if (contestSnap.exists) {
        ...
        if (!isLive) {
            throw new functionsV1.https.HttpsError("failed-precondition", end && end < now ? "This contest has ended" : "This contest hasn't started yet");
        }
    }
    ...
    const participants = allSnap.docs.map((d) => {
        ...
        return {
            ...
            ref: d.ref,
            score: data2.score ?? 0,
            completedAt: data2.quizCompletedAt?.toDate?.() ?? new Date(0),
        };
    });
    participants.sort((a, b) => b.score !== a.score ? b.score - a.score : a.completedAt.getTime() - b.completedAt.getTime());
