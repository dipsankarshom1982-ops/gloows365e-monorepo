...
exports.evaluateExam = exports.generateExam = void 0;
...
function buildExamPrompt(classLevel, board, subject, chapter, difficulty, language, questionCount) {
...
function buildEvaluationPrompt(examData, answers, classLevel, board) {
    const answerMap = new Map(answers.map((a) => [a.questionId, a.selectedIndex]));
    const results = examData.questions.map((q) => ({
        id: q.id,
        concept: q.concept,
...
Return ONLY a valid JSON object:
{
  ...
  "strongConcepts": ${JSON.stringify(results.filter((r) => r.correct).map((r) => r.concept))},
  "studyPlan": [
  ...
  "questionResults": ${JSON.stringify(results.map((r) => ({ id: r.id, correct: r.correct, selectedIndex: r.selectedIndex })))}
}`;
}
exports.generateExam = (0, https_1.onRequest)({
    timeoutSeconds: 90,
    memory: "512MiB",
    ...
    try {
        ...
        const key = `exam:gen:${uid}:${(0, redish_1.todayIST)()}`;
        let used = 0;
        try {
    ...
    // ── Cache check ──────────────────────────────────────────────────────────
    const cacheKey = `exam:gen:${(0, crypto_1.createHash)("sha256")
    ...
    // ── Generate exam ────────────────────────────────────────────────────────
    try {
        const prompt = buildExamPrompt(classLevel ?? "10", board ?? "CBSE", subject, chapter, difficulty, language, Math.min(Math.max(Number(questionCount), 10), 20));
        ...
        // ── Increment usage ──────────────────────────────────────────────────
        const usageKey = `exam:gen:${uid}:${(0, redish_1.todayIST)()}`;
        try {
        ...
        // Cache exam (1 hr)
        (0, redish_1.getRedis)().set(cacheKey, parsed, { ex: redish_1.TTL.discoverQuery }).catch(() => { });
...
exports.evaluateExam = (0, https_1.onRequest)({
    timeoutSeconds: 60,
    memory: "256MiB",
    secrets: ["GEMINI_API_KEY", "REDIS_URL", "REDIS_TOKEN"],
}, async (req, res) => {
    ...
    try {
        ...
        const raw = await (0, gemini_1.callGeminiText)(prompt);
        const evaluation = (0, gemini_1.parseJsonFromResponse)(raw);
...
//# sourceMappingURL=examSimulator.js.map