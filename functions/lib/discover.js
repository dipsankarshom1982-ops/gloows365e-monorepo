...
exports.discoverTrending = exports.discoverSearch = void 0;
...
// ─── Rate limiting ────────────────────────────────────────────────────────────
async function checkDiscoverLimit(uid) {
    const { isPremium } = await (0, usageCheck_1.getSubscription)(uid, db);
    if (isPremium)
        return -1; // unlimited
    const date = (0, redish_1.todayIST)();
    const key = redish_1.RK.discoverSearch(uid, date);
    let used = 0;
    try {
    ...
    if (used >= FREE_DAILY_DISCOVER) {
        throw new Error(`FREE_LIMIT_REACHED:You have used your ${FREE_DAILY_DISCOVER} free searches for today. Upgrade to Premium for unlimited discovery.`);
    }
    return FREE_DAILY_DISCOVER - used - 1;
}
async function incrementDiscoverUsage(uid) {
    const date = (0, redish_1.todayIST)();
    const key = redish_1.RK.discoverSearch(uid, date);
    try {
    ...
    }, { merge: true }).catch((e) => console.warn("discoverUsage write failed:", e));
}
// ─── Query hash ───────────────────────────────────────────────────────────────
function makeQueryHash(query, language) {
    return crypto
        .createHash("sha256")
...
async function runGoogleSearch(query) {
    ...
    try {
        ...
        const resp = await axios_1.default.get(`https://www.googleapis.com/customsearch/v1?${params}`, { timeout: 8000 });
...
function formatSnippetsForPrompt(results) {
    if (results.length === 0)
...
// ─── Gemini prompt ────────────────────────────────────────────────────────────
function buildDiscoverPrompt(query, studentName, classLevel, interests, language, collegeSnippets, scholarshipSnippets) {
...
// ─── History + trending ───────────────────────────────────────────────────────
async function saveSearchHistory(uid, query, hash) {
    const ref = db.collection("discoverHistory").doc(uid);
    await db.runTransaction(async (tx) => {
        const snap = await tx.get(ref);
        const existing = snap.exists ? (snap.data()?.searches ?? []) : [];
...
async function updateTrending(query) {
    try {
        const key = redish_1.RK.discoverTrending();
        const normalized = query.toLowerCase().trim().slice(0, 60);
        await (0, redish_1.getRedis)().zincrby(key, 1, normalized);
        (0, redish_1.getRedis)().expire(key, redish_1.TTL.discoverTrending).catch(() => { });
    }
    catch { /* Redis unavailable */ }
}
// ─── Export: discoverSearch ───────────────────────────────────────────────────
exports.discoverSearch = (0, https_1.onRequest)({
    timeoutSeconds: 60,
    memory: "512MiB",
    ...
    try {
        ...
        const { query, language = "English", studentName = "Student", classLevel = "10", interests = [], } = req.body;
        if (!query?.trim()) {
            res.status(400).json({ error: "query is required" });
            return;
        }
        const hash = makeQueryHash(query, language);
        const cacheKey = redish_1.RK.discoverQuery(hash);
        // ── 1. Redis cache check ──────────────────────────────────
        let cachedResult = null;
        try {
            cachedResult = await (0, redish_1.getRedis)().get(cacheKey);
        }
        catch { /* Redis unavailable */ }
        if (cachedResult) {
        ...
        if (fsSnap.exists) {
            const fsResult = fsSnap.data();
            (0, redish_1.getRedis)().set(cacheKey, fsResult, { ex: redish_1.TTL.discoverQuery }).catch(() => { });
            saveSearchHistory(uid, query, hash).catch(() => { });
            updateTrending(query).catch(() => { });
            await incrementDiscoverUsage(uid);
        ...
        // ── 4. Gemini AI call ────────────────────────────────────
        const prompt = buildDiscoverPrompt(query, String(studentName), classLevel, interests, language, formatSnippetsForPrompt(collegeResults), formatSnippetsForPrompt(scholarshipResults));
        const rawResponse = await (0, gemini_1.callGeminiText)(prompt);
        const parsed = (0, gemini_1.parseJsonFromResponse)(rawResponse);
        ...
        (0, redish_1.getRedis)().set(cacheKey, result, { ex: redish_1.TTL.discoverQuery }).catch(() => { });
...
exports.discoverTrending = (0, https_1.onRequest)({
    timeoutSeconds: 10,
    memory: "128MiB",
    ...
    try {
        ...
        const raw = await (0, redish_1.getRedis)().zrange(key, 0, 14, { rev: true, withScores: true });
        const terms = [];
