...
exports.ensureStudentId = void 0;
const admin = require("firebase-admin");
const functionsV1 = require("firebase-functions/v1");
const db = admin.firestore();
const ID_PREFIX = "GLS";
const ID_DIGITS = 6;
function formatStudentId(seq) {
    return `${ID_PREFIX}${String(seq).padStart(ID_DIGITS, "0")}`;
}
exports.ensureStudentId = functionsV1
    ...
    try {
        const studentId = await db.runTransaction(async (tx) => {
            ...
            const existing = studentSnap.exists ? studentSnap.data()?.studentId : undefined;
            if (existing)
                return existing;
