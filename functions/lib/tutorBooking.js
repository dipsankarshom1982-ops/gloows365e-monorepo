...
exports.cancelBooking = exports.respondToBooking = exports.requestBooking = void 0;
const admin = require("firebase-admin");
const functionsV1 = require("firebase-functions/v1");
const db = admin.firestore();
const SESSION_TYPES = ["trial", "regular"];
...
// ─── requestBooking ─────────────────────────────────────────────────────────
exports.requestBooking = functionsV1
    ...
    const { tutorUid, serviceId, subject, sessionType, requestedDate, requestedStartTime, requestedEndTime, } = data ?? {};
    ...
    if (serviceId) {
        ...
        if (serviceType === "instant_help") {
            ...
            throw new functionsV1.https.HttpsError("failed-precondition", "This service isn't bookable yet — Instant Help launches in a later phase");
        }
        ...
        resolvedSubject = service.subject ?? "";
        sessionFee = fee;
        serviceSnapshot = {
            serviceId,
        ...
        if (!anyService.empty) {
            throw new functionsV1.https.HttpsError("failed-precondition", "This tutor now offers services — please select a service to book");
        }
...
// ─── respondToBooking ───────────────────────────────────────────────────────
exports.respondToBooking = functionsV1
    .runWith({ timeoutSeconds: 30, memory: "256MB" })
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functionsV1.https.HttpsError("unauthenticated", "Login required");
...
exports.cancelBooking = functionsV1
    .runWith({ timeoutSeconds: 30, memory: "256MB" })
    .https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functionsV1.https.HttpsError("unauthenticated", "Login required");
...
//# sourceMappingURL=tutorBooking.js.map