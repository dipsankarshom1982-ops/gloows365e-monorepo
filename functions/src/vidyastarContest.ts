...
export const joinVidyastarContest = functionsV1
  ...
  .https.onCall(async (data: { contestId: string }, context) => {
    ...
    return db.runTransaction(async (tx) => {
      ...
      if (fee > 0 && balance < fee) {
        return { status: "insufficient_balance" as const, required: fee, balance };
      }
      ...
      return { status: "joined" as const, feePaid: fee };
    });
  });
