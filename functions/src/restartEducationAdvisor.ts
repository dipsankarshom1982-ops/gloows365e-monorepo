...
export const restartEducationAdvisor = functionsV1
  .runWith({
    timeoutSeconds: 60,
    memory: "256MB",
    secrets: ["GEMINI_API_KEY"],
  })
  .https.onCall(async (data: { message: string }, context) => {
    if (!context.auth) {
    ...
    try {
      ...
      if (chatSnap.exists) {
        const msgs = (chatSnap.data()?.messages ?? []) as any[];
...
Advisor:`;
    ...
    let reply = "";
    try {
      const raw = await callGeminiText(fullPrompt);
    ...
    try {
      ...
      const messageCount    = Math.floor(updatedMessages.filter((m: any) => m.role === "user").length);
