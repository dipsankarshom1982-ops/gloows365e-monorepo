...
import { assignRanks, type RankableParticipant } from "./contestRankingPolicy";
...
export type FinalizeOutcome =
  ...
  | { status: "finalized"; participantCount: number; chunksWritten: number; chunksSkipped: number };
...
export async function finalizeContest(contestId: string): Promise<FinalizeOutcome> {
  const contestRef = db.doc(`contests/${contestId}`);
  const contestSnap = await contestRef.get();
  if (!contestSnap.exists) return { status: "contest_not_found" };
  if (contestSnap.data()?.leaderboardFinalized === true) return { status: "already_finalized" };
  ...
  const participants: (RankableParticipant & { existingFinalRank?: number })[] = [];
...
export const finalizeContestRanking = functionsV1
...
export const autoFinalizeEndedContests = onSchedule(
  { schedule: "every 15 minutes", timeZone: "Asia/Kolkata", timeoutSeconds: 300, memory: "256MiB" },
  async () => {
    const now = new Date();
    const snap = await db.collection("contests").get();
    const toFinalize = snap.docs.filter((d) => {
      const c = d.data();
      if (c.leaderboardFinalized === true) return false;
