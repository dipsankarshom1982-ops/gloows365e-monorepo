...
const FIELD_KEYED_COLLECTIONS: { collection: string; field: string }[] = [
...
async function collectDocKeyed(uid: string): Promise<Record<string, unknown>> {
  const out: Record<string, unknown> = {};
  for (const col of DOC_KEYED_COLLECTIONS) {
    try {
      const snap = await db.collection(col).doc(uid).get();
      if (snap.exists) out[col] = snap.data();
    } catch (err) {
...
async function collectFieldKeyed(uid: string): Promise<Record<string, unknown[]>> {
  const out: Record<string, unknown[]> = {};
  for (const { collection, field } of FIELD_KEYED_COLLECTIONS) {
    try {
      const snap = await db.collection(collection).where(field, "==", uid).get();
      if (!snap.empty) out[collection] = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
    } catch (err) {
...
async function deleteDocKeyed(uid: string): Promise<void> {
  for (const col of DOC_KEYED_COLLECTIONS) {
    try {
...
async function deleteFieldKeyed(uid: string): Promise<void> {
  for (const { collection, field } of FIELD_KEYED_COLLECTIONS) {
    try {
      const snap = await db.collection(collection).where(field, "==", uid).get();
...
async function deleteAdAnalyticsEvents(uid: string): Promise<void> {
  try {
    const snap = await db
...
async function scrubRetainedRecords(uid: string): Promise<void> {
  for (const col of SCRUB_ONLY_COLLECTIONS) {
    try {
      ...
      const scrubbed: Record<string, admin.firestore.FieldValue> = {};
      for (const field of ["name", "email", "phone", "contactEmail", "contactPhone"]) {
        if (field in data) scrubbed[field] = admin.firestore.FieldValue.delete();
      }
...
export const exportMyData = functionsV1
...
export const eraseMyAccount = functionsV1
  .runWith({ timeoutSeconds: 120, memory: "256MB" })
  .https.onCall(async (_data: unknown, context) => {
    if (!context.auth) {
      throw new functionsV1.https.HttpsError("unauthenticated", "Login required");
