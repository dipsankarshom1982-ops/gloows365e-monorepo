...
export interface QuizAnswerKeyEntry { correctAnswerIndex: number; explanation: string; }

export function deriveAnswerKey(rawQuiz: any[], context: string): QuizAnswerKeyEntry[] {
...
export function sanitizeQuizForClient(rawQuiz: any[]): any[] {
...
export function quizLooksUnsplit(rawQuiz: unknown): boolean {
  return Array.isArray(rawQuiz) && rawQuiz.some((q: any) => q?.correctAnswerIndex !== undefined);
}
