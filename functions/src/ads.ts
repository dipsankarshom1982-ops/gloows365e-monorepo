...
import { getRedis, TTL, RK, todayIST } from "./redish";
...
function isSafeAd(ad: Record<string, any>): boolean {
  if (!ALLOWED_CATEGORIES.includes(ad.adCategory)) return false;
...
export const getAds = functionsV1
  .runWith({ timeoutSeconds: 15, memory: "128MB", secrets: ["REDIS_URL", "REDIS_TOKEN"] })
  .https.onCall(async (
    data: { module: string; classLevel?: string; adType?: string; limit?: number },
    context
  ) => {
    if (!context.auth) {
    ...
    const cacheKey  = RK.ads(module, cls);

    // ── Check Redis cache ──
    try {
      const cached = await getRedis().get<unknown[]>(cacheKey);
      if (cached) {
        let result = cached as Record<string, any>[];
        if (adType) result = result.filter((a) => a.adType === adType);
    ...
    let ads = snap.docs
      ...
      .filter((ad: any) => {
        if (ad.isApproved !== true) return false;
        // Module targeting
        const targetModules: string[] = ad.targetModules ?? [];
        if (!targetModules.includes(module) && !targetModules.includes("all")) return false;
        // Class targeting
        const targetClass: string[] = ad.targetClass ?? [];
        if (!targetClass.includes(cls) && !targetClass.includes("all")) return false;
        // Date range
        if (ad.startDate && ad.startDate.toMillis() > now.toMillis()) return false;
        if (ad.endDate   && ad.endDate.toMillis()   < now.toMillis()) return false;
        // Safety
        return isSafeAd(ad);
      })
    ...
    try { getRedis().set(cacheKey, ads, { ex: TTL.ads }).catch(() => {}); } catch { /* ok */ }

    // Apply adType filter after caching
    if (adType) ads = ads.filter((a: any) => a.adType === adType);
...
export const recordAdEvent = functionsV1
  .runWith({ timeoutSeconds: 10, memory: "128MB", secrets: ["REDIS_URL", "REDIS_TOKEN"] })
  .https.onCall(async (
    data: { adId: string; event: string; module: string; sessionId: string; classLevel?: string },
    context
  ) => {
    if (!context.auth) {
      throw new functionsV1.https.HttpsError("unauthenticated", "Login required");
    ...
    if (!adId || !ALLOWED_EVENTS.includes(event as any)) {
    ...
    const dedupKey = RK.adEvent(uid, adId, event);
    try {
      const exists = await getRedis().get(dedupKey);
      if (exists) return { success: true, deduped: true };
      await getRedis().set(dedupKey, 1, { ex: TTL.adEvents });
...
export const claimAdReward = functionsV1
  .runWith({ timeoutSeconds: 15, memory: "128MB", secrets: ["REDIS_URL", "REDIS_TOKEN"] })
  .https.onCall(async (
    data: { adId: string; sessionId: string },
    context
  ) => {
    if (!context.auth) {
      throw new functionsV1.https.HttpsError("unauthenticated", "Login required");
    ...
    const rewardKey = RK.adReward(uid, adId);
    try {
      const already = await getRedis().get(rewardKey);
      if (already) {
    ...
    const freqKey  = RK.adFreq(uid, today);
    let todayCount = 0;

    try {
      const val = await getRedis().get<number>(freqKey);
      todayCount = val ?? 0;
    ...
    const ad = adSnap.data() as any;
    const coins: number = ad.reward?.coins ?? 0;
    ...
    await db.runTransaction(async (tx) => {
      ...
      const current = (balSnap.data()?.balance ?? 0) as number;
    ...
    try {
      await getRedis().incr(freqKey);
      await getRedis().expire(freqKey, ttlUntilMidnight);
      await getRedis().set(rewardKey, 1, { ex: ttlUntilMidnight });
    } catch { /* Redis unavailable */ }

    // ── 7. Invalidate vcoin balance cache ──
    getRedis().del(`vcoin:balance:${uid}`).catch(() => {});
...
export const aggregateAdAnalytics = functionsV1
  ...
  .onRun(async () => {
    ...
    await Promise.all(adsSnap.docs.map(async (adDoc) => {
      ...
      const moduleCounts: Record<string, { impressions: number; clicks: number }> = {};
      const classCounts:  Record<string, { impressions: number }> = {};
      ...
      eventsSnap.docs.forEach((e) => {
        ...
        if (!moduleCounts[mod]) moduleCounts[mod] = { impressions: 0, clicks: 0 };
        if (!classCounts[cls])  classCounts[cls]  = { impressions: 0 };

        switch (ev.event) {
          case "impression":     impressions++;     moduleCounts[mod].impressions++; classCounts[cls].impressions++; break;
          case "click":          clicks++;           moduleCounts[mod].clicks++; break;
          case "watch_complete": watchComplete++;    break;
          case "reward_claimed": rewardsClaimed++;   break;
        }
      });
