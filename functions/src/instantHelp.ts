...
type InstantHelpRequestStatus = "pending" | "accepted" | "declined" | "expired" | "cancelled";
type InstantHelpSessionStatus = "active" | "ended";
type InstantHelpEndReason = "student_ended" | "tutor_ended" | "insufficient_balance";

const TUTOR_ROLE_CLAIMS = ["TUTOR", "TEACHER", "COACHING_CENTER"];
const REQUEST_TIMEOUT_MS = 60_000; // tutor has 60s to accept/decline
...
export const setInstantHelpOnlineStatus = functionsV1
  .runWith({ timeoutSeconds: 15, memory: "128MB" })
  .https.onCall(async (data: { online?: boolean }, context) => {
    if (!context.auth) {
    ...
    if (data.online) {
      ...
      if (svc.empty) {
        throw new functionsV1.https.HttpsError(
          "failed-precondition",
          "Publish an Instant Help service before going online"
        );
      }
    }
...
// ─── requestInstantHelp ─────────────────────────────────────────────────────
export const requestInstantHelp = functionsV1
  .runWith({ timeoutSeconds: 30, memory: "256MB" })
  .https.onCall(async (
    data: { tutorUid?: string; serviceId?: string },
    context
  ) => {
    if (!context.auth) {
      throw new functionsV1.https.HttpsError("unauthenticated", "Login required");
    ...
    const result = await db.runTransaction(async (tx) => {
      ...
      const [
        studentPendingSnap, studentActiveSnap,
        tutorPendingSnap, tutorActiveSnap,
        balanceSnap,
      ] = await Promise.all([
      ...
      const studentHasLivePending = studentPendingSnap.docs.some(
      ...
      const tutorHasLivePending = tutorPendingSnap.docs.some(
        (d) => (d.data().expiresAt?.toMillis?.() ?? 0) > now
      );
      ...
      if (balance < creditsPerMinute) {
        throw new functionsV1.https.HttpsError(
          "failed-precondition",
          `You need at least ${creditsPerMinute} credit${creditsPerMinute === 1 ? "" : "s"} to start — your balance is ${balance}`
        );
      }
      ...
      tx.set(requestRef, {
        ...
        tutorUid,
        serviceId,
        subject: (service.subject as string) ?? "",
        topics: Array.isArray(service.topics) ? service.topics : [],
        creditsPerMinute,
        status: "pending" as InstantHelpRequestStatus,
...
export const respondToInstantHelpRequest = functionsV1
  .runWith({ timeoutSeconds: 30, memory: "256MB" })
  .https.onCall(async (
    data: { requestId?: string; action?: "accepted" | "declined" },
    context
  ) => {
    if (!context.auth) {
      throw new functionsV1.https.HttpsError("unauthenticated", "Login required");
    ...
    // Throwing inside a db.runTransaction() callback aborts the WHOLE
    // transaction — nothing queued via tx.update()/tx.set() up to that
    // point ever commits, even if the throw comes after the write call.
    // So every "reject this and also record why" branch below returns a
    // discriminated result instead of throwing mid-transaction — the
    // write commits normally, and the HttpsError is thrown once, AFTER
    // the transaction, from the result the caller actually got back.
    type Outcome =
      | { kind: "not_found" }
      | { kind: "wrong_tutor" }
    ...
    const outcome: Outcome = await db.runTransaction(async (tx) => {
      ...
      if (request.tutorUid !== tutorUid) return { kind: "wrong_tutor" };
      if (request.status !== "pending") return { kind: "already_resolved", status: request.status };

      const now = Date.now();
      const expiresAtMs: number = request.expiresAt?.toMillis?.() ?? 0;
      if (now > expiresAtMs) {
        tx.update(requestRef, {
          status: "expired" as InstantHelpRequestStatus,
          updatedAt: admin.firestore.FieldValue.serverTimestamp(),
        });
        return { kind: "expired" };
      }
      ...
      if (action === "declined") {
        tx.update(requestRef, {
          status: "declined" as InstantHelpRequestStatus,
      ...
      tx.set(sessionRef, {
        ...
        creditsPerMinute,
        status: "active" as InstantHelpSessionStatus,
      ...
      tx.update(requestRef, {
        status: "accepted" as InstantHelpRequestStatus,
        respondedAt: nowFv,
        updatedAt: nowFv,
      });
...
export const cancelInstantHelpRequest = functionsV1
  .runWith({ timeoutSeconds: 15, memory: "128MB" })
  .https.onCall(async (data: { requestId?: string }, context) => {
    ...
    const result = await db.runTransaction(async (tx) => {
      ...
      tx.update(requestRef, {
        status: "cancelled" as InstantHelpRequestStatus,
        respondedAt: admin.firestore.FieldValue.serverTimestamp(),
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
      });
      return { status: "cancelled" as InstantHelpRequestStatus };
    });
...
async function settleInstantHelpSession(
  ...
  const result: SettleResult = await db.runTransaction(async (tx) => {
    ...
    if (session.status !== "active") {
      return { status: session.status as InstantHelpSessionStatus, endReason: session.endReason, minutesCharged: 0 };
    }

    const creditsPerMinute = Number(session.creditsPerMinute) || 0;
    const lastBilledAt: admin.firestore.Timestamp = session.lastBilledAt;
    const nowTs = admin.firestore.Timestamp.now();
    const elapsedMinutes = Math.max(0, Math.floor((nowTs.toMillis() - lastBilledAt.toMillis()) / 60000));

    let minutesToCharge = elapsedMinutes;
    let endReason: InstantHelpEndReason | undefined = opts?.manualEndReason;
    ...
    if (minutesToCharge === 0 && !willEnd) {
      ...
      return { status: "active" as InstantHelpSessionStatus, minutesCharged: 0 };
    }
    ...
    if (willEnd) {
      patch.status = "ended" as InstantHelpSessionStatus;
...
export const endInstantHelpSession = functionsV1
  .runWith({ timeoutSeconds: 30, memory: "256MB" })
  .https.onCall(async (data: { sessionId?: string }, context) => {
    ...
    const manualEndReason: InstantHelpEndReason =
      pre.studentUid === callerUid ? "student_ended" : "tutor_ended";
...
export const tickInstantHelp = functionsV1
  ...
  .onRun(async () => {
    ...
    const settleResults = await Promise.allSettled(
      activeSnap.docs.map((d) => settleInstantHelpSession(d.id))
    );
